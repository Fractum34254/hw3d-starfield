...
namespace Bind
{
	...
	{
	public:
		Texture( Graphics& gfx, int resource, UINT slot = 0);
		...
		static std::shared_ptr<Texture> Resolve(Graphics& gfx, int res, UINT slot = 0);
		static std::string GenerateUID( const std::string& path,UINT slot = 0 );
		static std::string GenerateUID(int res, UINT slot = 0);
