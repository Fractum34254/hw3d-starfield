...
namespace Bind
{
	...
	Texture::Texture(Graphics & gfx, int resource, UINT slot)
		:
		path("Resource" + std::to_string(resource)),
		slot(slot)
	{
		INFOMAN(gfx);

		// load surface
		const auto s = Surface::FromResource(resource);
	...
	std::shared_ptr<Texture> Texture::Resolve(Graphics & gfx, int res, UINT slot)
	{
		return Codex::Resolve<Texture>(gfx, res, slot);
	}
	...
	std::string Texture::GenerateUID(int res, UINT slot)
	{
		using namespace std::string_literals;
		return typeid(Texture).name() + "#"s + std::to_string(res) + "#" + std::to_string(slot);
	}
